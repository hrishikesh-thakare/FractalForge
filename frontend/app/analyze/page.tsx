  ...
  const handleAnalyzeClick = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        setError(`Failed to analyze the image. Please ensure the backend server is running. ${err.message}`)
      } else {
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <main className="relative z-10 container mx-auto p-4 md:p-8 pt-24">
        <div className="text-center mb-12 floating-element">
          <h1 className="text-5xl md:text-6xl font-bold text-foreground mb-4 text-balance">Kolam & Rangoli Analyzer</h1>
          <p className="text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto text-pretty">
            Discover the mathematical beauty in traditional art. Upload your Kolam or Rangoli design to analyze its
            geometric patterns and cultural significance.
          </p>
            ...
            <CardContent>
              <div className="flex flex-col items-center gap-6">
                <div className="w-full">
                  ...
                  />
                  <p className="text-sm text-muted-foreground mt-2 text-center">Supports PNG, JPEG, and JPG formats</p>
                </div>
            ...
            <CardContent className="min-h-[400px]">
              {isLoading && (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                  ...
                  <p className="text-lg text-muted-foreground animate-pulse">Analyzing your beautiful design...</p>
                </div>
              )}
              ...
              {analysis ? (
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-primary/20 to-secondary/20 border-2 border-primary/30 rounded-2xl p-6 text-center">
                    ...
                    <p className="text-3xl font-bold text-primary">{analysis.classification}</p>
                  </div>

                  {/* Detailed metrics */}
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-card/50 border border-border rounded-xl p-4 text-center hover:bg-card/70 transition-colors">
                      <div className="text-2xl font-bold text-foreground">{analysis.dots}</div>
                      <div className="text-sm text-muted-foreground font-medium">Detected Dots</div>
                    </div>
                    <div className="bg-card/50 border border-border rounded-xl p-4 text-center hover:bg-card/70 transition-colors">
                      <div className="text-2xl font-bold text-foreground">{analysis.curves}</div>
                      <div className="text-sm text-muted-foreground font-medium">Detected Curves</div>
                    </div>
                    <div className="bg-card/50 border border-border rounded-xl p-4 text-center hover:bg-card/70 transition-colors">
                      <div className="text-2xl font-bold text-foreground">{analysis.closed_loops}</div>
                      <div className="text-sm text-muted-foreground font-medium">Closed Loops</div>
                    </div>
                    <div className="bg-card/50 border border-border rounded-xl p-4 text-center hover:bg-card/70 transition-colors">
                      <div className="text-2xl font-bold text-foreground">{analysis.circularity.toFixed(3)}</div>
                      <div className="text-sm text-muted-foreground font-medium">Circularity Score</div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-accent/10 to-secondary/10 border border-accent/30 rounded-xl p-6">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Your {analysis.classification.toLowerCase()} design showcases the mathematical precision inherent
                      in traditional Indian art forms. The {analysis.dots} dots and {analysis.curves} curves create a
                      harmonious pattern that reflects centuries of cultural wisdom.
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Upload a Kolam or Rangoli image to discover its geometric patterns and cultural significance.
                      </p>
                    </div>
                  </div>
                )
        ...
        <div className="mt-16 max-w-4xl mx-auto">
          <Card className="glass-morphism border border-border/50">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-foreground mb-6 text-center">About the Analyzer</h2>
              <div className="grid md:grid-cols-3 gap-6 text-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Advanced algorithms detect dots, curves, and geometric patterns in your designs.
                  </p>
                </div>
                <div>
                  ...
                  <h3 className="font-semibold text-foreground mb-2">Mathematical Analysis</h3>
                  <p className="text-sm text-muted-foreground">
                    Calculates circularity scores and identifies structural elements of traditional art.
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Provides insights into the cultural significance and artistic traditions.
                  </p>
                </div>
              </div>
            </CardContent>
